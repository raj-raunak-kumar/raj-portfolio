                ...
                {timelineItems.map((item, index) => (
                    <div key={index} className={`flex items-center w-full mb-12 flex-row scroll-animate ${item.side === 'right' ? 'md:flex-row-reverse' : ''}`} style={{ animationDelay: `${index * 150}ms` }}>
                        ...
                        <div className={`w-5/12 flex items-center ${item.side === 'left' ? 'justify-start' : 'justify-end'}`}>
                            <div className={`p-6 bg-black/60 border rounded-2xl hover:bg-opacity-10 transition-all duration-300 w-full max-w-[260px] transform hover:scale-105 hover:shadow-[0_0_20px_rgba(0,204,255,0.2)] aspect-square flex flex-col justify-center ${item.side === 'left' ? 'md:text-right md:items-end' : 'md:text-left md:items-start'}`} style={{ borderColor: `${modeColor}4D`, background: `rgba(0, 204, 255, 0.05)` }}>
                                <h3 className="text-xl font-bold font-mono mb-1" style={{ color: modeColor }}>{item.year}</h3>
                                <h4 className="text-white text-base font-bold mb-2 leading-snug">{item.title}</h4>
